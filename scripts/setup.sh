...
declare -A service_containers=(
    ["all"]="nginx rag reranker prompt ollama standalone etcd etcd_init minio meilisearch vision"
    ["all-gpu"]="nginx rag reranker prompt ollama-gpu vllm standalone etcd etcd_init minio meilisearch vision"
    ["llm-full"]="nginx rag reranker prompt llm-server vllm standalone etcd etcd_init minio meilisearch vision"
    ["rag"]="nginx rag standalone etcd etcd_init minio meilisearch"
    ["reranker"]="nginx reranker"
    ["prompt"]="nginx prompt"
    ["rag-reranker"]="nginx rag reranker standalone etcd etcd_init minio meilisearch"
    ["db"]="standalone etcd etcd_init minio meilisearch"
    ["app-only"]="nginx rag reranker prompt ollama meilisearch vision"
    ["app-only-gpu"]="nginx rag reranker prompt ollama-gpu vllm meilisearch vision"
    ["ollama"]="ollama"
